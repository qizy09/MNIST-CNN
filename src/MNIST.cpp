...
#define startWith 34810
#define iterationNumber 40000
...
{
    //Read the targets
	std::ifstream targetFile(string("/home/ziyang/Desktop/data/") + "train-labels.idx1-ubyte", std::ios::binary);
//	cout << "hello0" << endl;
	...
	std::ifstream trainingFeatureFile(string("/home/ziyang/Desktop/data/") + "train-images.idx3-ubyte", std::ios::binary);
...
{
    //Read the targets
	std::ifstream targetFile(string("/home/ziyang/Desktop/data/") + "t10k-labels.idx1-ubyte", std::ios::binary);
    if (targetFile.is_open())
    {
        int magic_number=0;
        int number_of_images=0;
	...
	std::ifstream trainingFeatureFile(string("/home/ziyang/Desktop/data/") + "t10k-images.idx3-ubyte", std::ios::binary);
    if (trainingFeatureFile.is_open())
    {
        int magic_number=0;
        int number_of_images=0;
...
void trainMNISTFullyConnectedNN()
{
	...
	cout << endl<< inputs.size() << " " << inputSize << endl;
	cout << "Finished reading. Now creating neural net" << endl;
    for (int i = 0; i < 10; ++i)
        cout << targets[0][i]<< endl;
    cout << endl;
    ...
    //Create a context
    cl_context_properties cps[3] = {CL_CONTEXT_PLATFORM, (cl_context_properties)(*platforms.begin())(), 0};
    cl::Context context = cl::Context(CL_DEVICE_TYPE_CPU, cps);

    //Create and build the program
    cl::Program fullyConnectedNeuralNetProgram;
	fullyConnectedNeuralNetProgram = createProgram(context, string("/home/ziyang/Desktop/OpenCLNeuralNet/") + "fullyconnectedneuralnet.cl");
...
void testMNISTFullyConnectedNN()
{
    ...
    for (int i = 0 ; i < 51; i += 1)//Iterates through neural nets and tests it against the testing set
    {
		...
		fileNameStream<< "/home/ziyang/Desktop/data/" << "NN-" << i << ".net";//"/home/ziyang/Desktop/OpenCLNeuralNet/"
...
void trainMNISTConvolutionalNN()
{
	...
	convolutionalNeuralNetProgram = createProgram(context, string("/home/ziyang/Desktop/OpenCLNeuralNet/") + "convolutionalneuralnet.cl");
    ...
    int inputDim = 28;
    int outputDim = (inputDim - filterDim + 1) / MAXPOOLDIM; //24*24*5
    ...
    std::ostringstream fileNameStream;
    fileNameStream << "/home/ziyang/Desktop/data/"<< "CNN-" << startWith - 1 << ".net";
    (*myNet).loadNeuralNetworkFromFile(fileNameStream.str(), context, fullyConnectedNeuralNetProgram, convolutionalNeuralNetProgram);
    //(*myNet).calculateError(inputs,targets,&queue);
    (*myNet).trainNeuralNet(inputs, targets, &queue, startWith, iterationNumber);
//    (*myNet).writeNeuralNetworkToFile(queue);
...
#else
    //Iteratively loads a NN from a folder and tests to see how good it is
    for (int i = startWith ; i < iterationNumber; ++i)
    	if (i % 50 == 9)
    {
        std::ofstream netFile;
        std::ostringstream fileNameStream;
        fileNameStream << "/home/ziyang/Desktop/data/"<< "CNN-" << i << ".net";
        cout << "TESTING NET " << fileNameStream.str() << endl;
        NeuralNetwork *myNet = new NeuralNetwork; 
        (*myNet).loadNeuralNetworkFromFile(
            fileNameStream.str(),
